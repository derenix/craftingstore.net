...
#region User hat sich eingeloggt, es wird ein Token übergeben
	if ($_GET['tok'] && isNumber($_GET['uid'])) {
		$uid = $_GET['uid'];
		$token = MySqlDatabase::getInstance()->fetchOne("SELECT Token FROM mc_gamer WHERE Id= ? LIMIT 1", array("i" => $uid));

		if ($_GET['tok'] === $token) {
			MySqlDatabase::getInstance()->query("UPDATE mc_gamer FSET Token='' WHERE Id= ? LIMIT 1", array("i" => $uid));

			$subdomain = getCurrentSubdomain();
			if ($subdomain == 'secure') {
				if (MySqlDatabase::getInstance()->fetchOne("SELECT Validated FROM mc_gamer WHERE Id= ? LIMIT 1", array("i" => $uid))) {
					$_SESSION['Index']->user = new User($uid);
				}
			} else {
				...
				if (MySqlDatabase::getInstance()->fetchOne("SELECT g.validated FROM mc_gamer AS g
INNER JOIN mc_permittedshops AS p ON g.Id=p.GamerId
INNER JOIN mc_shops AS s ON s.Id = p.ShopId
where g.Id= ? AND (Domain= ? OR Subdomain= ?) LIMIT 1", array("i" => $uid, "s" => $domain, "s" => $subdomain))
				) {
...
#region Redirect durchführen
	if (isset($_GET['red'])) {
		...
		if ($_SESSION['Index']->user->isLoggedIn()) {
			...
			if (isNumber($shop) && ($domain = MySqlDatabase::getInstance()->fetchOneRow("SELECT Subdomain,Domain FROM mc_shops WHERE Id='$shop' LIMIT 1"))) {
