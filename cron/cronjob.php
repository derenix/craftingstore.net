...
if (!MySqlDatabase::getInstance()->isConnected()) {
	try {
		MySqlDatabase::getInstance()->connect(SQL_HOST, SQL_USERNAME, SQL_PASSWORD, SQL_DB);
		MySqlDatabase::getInstance()->query("SET NAMES 'utf8'");
	} catch (Exception $e) {
		die($e->getMessage());
	}
}
...
if (count($_SERVER['argv']) > 1) {
	...
	foreach (MySqlDatabase::getInstance()->query("SELECT UserId FROM mc_transferCommands WHERE ShopId='$ShopId' AND Type='BY_CRON' AND IgnoreCommand<>1 AND Locked='0'
	...
	MySqlDatabase::getInstance()->update("UPDATE mc_shops SET CronLock='0' WHERE Id='$ShopId' LIMIT 1");
	...
	$anz = MySqlDatabase::getInstance()->update("UPDATE mc_shops AS s SET s.CronLock='$uuid' WHERE s.CronLock='0' AND s.ServerOnline='1'
	...
	foreach (MySqlDatabase::getInstance()->query("SELECT Id FROM mc_shops WHERE CronLock='$uuid'") as $row) {
