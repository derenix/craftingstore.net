...
class ItemEdit extends aDisplayable
{
	...
	private static function updateEnchList($ShopId, $selectedEnchesArray, $selectedEnchesStrength, &$newEnchId, &$newEnchStrength, $addButton, $deleteButton, &$fullEnchList)
	{
		...
		$fullEnchList = array();
		// foreach(MySqlDatabase::getInstance()->query("SELECT Id, Name FROM mc_ench WHERE (ShopId='$ShopId' OR (ShopId='0' AND Id NOT IN (SELECT Id FROM mc_items WHERE ShopId='$ShopId'))) ORDER BY ShopId ASC, Name ASC") as $row){
		foreach (MySqlDatabase::getInstance()->query("SELECT Id, Name FROM mc_ench WHERE ShopId='$ShopId' OR ShopId='0' ORDER BY Name ASC") as $row) {
	...
	public static function isValidData($ShopId, $ItemId, $itemEditName, $itemEditMineId, $itemEditValue, &$noName = false, &$invalidMineId = false, &$nameInUse = false)
	{
		if (!$itemEditName) {
			...
			$invalidMineId = true;
		} elseif (MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_items WHERE name='" . mysql_real_escape_string($itemEditName) . "' AND Id<>'{$ItemId}' AND ShopId='$ShopId' LIMIT 1")) {
	...
	public function prepareDisplay()
	{
		...
		$Minecraftname = MySqlDatabase::getInstance()->fetchOne("SELECT Minecraftname FROM mc_customers INNER JOIN mc_shops ON mc_shops.CustomersId=mc_customers.Id WHERE mc_shops.Id='{$_SESSION['Index']->adminShop->getId()}' LIMIT 1");
		$itemEditSaveButton = $_POST['itemEditSaveButton'];
		$itemEditTestButton = $_POST['itemEditTestButton'];

		if (!isset($_POST['itemEditName'])) {
			$itemInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_items WHERE Id='{$this->ItemId}' AND ShopId='$ShopId' LIMIT 1");
			...
			foreach (MySqlDatabase::getInstance()->query("SELECT EnchId,Strength FROM mc_EnchInItem WHERE ShopId='$ShopId' AND ItemId='{$this->ItemId}'") as $row) {
			...
			if (ItemEdit::isValidData($ShopId, $this->ItemId, $itemEditName, $itemEditMineId, $itemEditValue, $noName, $invalidMineId, $nameInUse)) {
				...
				if (is_array($selectedEnches)) {
					foreach ($selectedEnches as $value) {
						if (isNumber($value['Id']) && isNumber($value['Amount']) && $enchName = MySqlDatabase::getInstance()->fetchOne("SELECT Name FROM mc_ench WHERE (ShopId='0' OR ShopId='$ShopId') AND Id='" . mysql_real_escape_string($value['Id']) . "'")) {
			...
			try {
				...
				if (ItemEdit::isValidData($ShopId, $this->ItemId, $itemEditName, $itemEditMineId, $itemEditValue, $noName, $invalidMineId, $nameInUse)) {
					...
					if ($this->ItemId == 0) { //Neu erstellen
						$createNew = true;
						$this->ItemId = MySqlDatabase::getInstance()->insert("INSERT INTO mc_items (ShopId,Name,Ingame,MineId,Damage,Lore) VALUES ('$ShopId','" . mysql_real_escape_string($itemEditName) . "','" . mysql_real_escape_string($itemEditIngame) . "','$itemEditMineId',$itemEditValue,'" . mysql_real_escape_string($itemEditLore) . "')");
					} else { //altes aktualisieren
						$createNew = false;
						MySqlDatabase::getInstance()->insert("UPDATE mc_items SET Name='" . mysql_real_escape_string($itemEditName) . "',Ingame='" . mysql_real_escape_string($itemEditIngame) . "',MineId='$itemEditMineId',Damage=$itemEditValue,Lore='" . mysql_real_escape_string($itemEditLore) . "' WHERE ShopId='$ShopId' AND Id='{$this->ItemId}' LIMIT 1");
						MySqlDatabase::getInstance()->query("DELETE FROM mc_EnchInItem WHERE ShopId='$ShopId' AND ItemId='{$this->ItemId}'");
					}
					$insertEnches = '';
					if (is_array($selectedEnches)) {
						foreach ($selectedEnches as $value) {
					...
					if ($insertEnches) MySqlDatabase::getInstance()->insert('INSERT INTO mc_EnchInItem (ShopId,ItemId,EnchId,Strength) VALUES ' . $insertEnches);
					commit();
