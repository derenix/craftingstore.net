	...
	public function prepareDisplay()
	{
		...
		function moveUpAllowed($IdToMove, $CurrentPosition)
		{
			if (!isNumber($IdToMove) || !isNumber($CurrentPosition))
				return false;
			return MySqlDatabase::getInstance()->fetchOne("SELECT Position FROM mc_topmenu WHERE Id='$IdToMove' AND Position='$CurrentPosition' AND ShopId='{$_SESSION['Index']->adminShop->getId()}'") > 1;
		}
		...
		function moveDownAllowed($IdToMove, $CurrentPosition)
		{
			if (!isNumber($IdToMove) || !isNumber($CurrentPosition))
				return false;
			return MySqlDatabase::getInstance()->fetchOne("SELECT Position FROM mc_topmenu WHERE Id='$IdToMove' AND Position='$CurrentPosition' AND ShopId='{$_SESSION['Index']->adminShop->getId()}'") < MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_topmenu WHERE ShopId='{$_SESSION['Index']->adminShop->getId()}'");
		}
		...
		if (count($_POST)) {
			...
			if ($fehler) {
				...
				if ($_GET['id'] == -1) {
					$newPos = MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_topmenu WHERE ShopId='$currShop'") + 1;
					MySqlDatabase::getInstance()->insert("INSERT INTO mc_topmenu (ShopId, Name, Link, Position, Target) VALUES ('$currShop', '$name', '$link', '$newPos', '$target')");
				} else {
					MySqlDatabase::getInstance()->update("UPDATE mc_topmenu SET Name='$name', Link='$link', Target='$target' WHERE Id='$id'");
				}
				$saved = true;
			}
		}
		...
		if (moveUpAllowed($_GET['moveup'], $_GET['pos'])) {
			MySqlDatabase::getInstance()->query("LOCK TABLES mc_topmenu WRITE");
			$idToMove = $_GET['moveup'];
			$posOfMove = MySqlDatabase::getInstance()->fetchOne("SELECT Position FROM mc_topmenu WHERE Id='$idToMove'");
			$posOfMoveM = $posOfMove - 1;
			MySqlDatabase::getInstance()->update("UPDATE mc_topmenu SET Position='$posOfMove' WHERE ShopId='$currShop' AND Position='$posOfMoveM'");
			MySqlDatabase::getInstance()->update("UPDATE mc_topmenu SET Position='$posOfMoveM' WHERE Id='$idToMove'");
		...
		if (isNumber($_GET['delete'])) {
			...
			$oldPosition = MySqlDatabase::getInstance()->fetchOne("SELECT Position FROM mc_topmenu WHERE Id='$IdToDelete'");
			MySqlDatabase::getInstance()->query("DELETE FROM mc_topmenu WHERE Id='$IdToDelete'");
			MySqlDatabase::getInstance()->query("UPDATE mc_topmenu SET Position=Position-1 WHERE ShopId='$currShop' AND Position>'$oldPosition'");
		}
		...
		if ((isNumber($_GET['id']) || $_GET['id'] == -1 || $fehler) && !$saved) {
			...
			if ($_GET['id'] == -1) {
				...
				if (count($_POST)) {
					...
					$row = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Name, Link, Target FROM mc_topmenu WHERE Id='$idToEdit'");
		...
		$number = MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_topmenu WHERE ShopId='$currShop'");

		$this->links = array();
		$sql = "SELECT Id, Name, Link, Target, Position FROM mc_topmenu WHERE ShopId='$currShop' ORDER BY Position";

		$n = 1;
		foreach (MySqlDatabase::getInstance()->query($sql) as $row) {
			$first = false;
			$last = false;
			if ($n == 1) $first = true;
