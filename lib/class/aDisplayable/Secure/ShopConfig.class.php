...
class ShopConfig extends aDisplayable // todo: Bildgröße in die Datenbank reinschreiben
{
	public function prepareDisplay()
	{
		...
		$old_logo = MySqlDatabase::getInstance()->fetchOne("SELECT ShopLogo FROM mc_shops WHERE Id = '" . $_SESSION['Index']->adminShop->getShopInfo()->Id . "' LIMIT 1");


		if (isset($_POST['submit'])) {
			if (isNumber($_POST['starting_credit'])) {
				MySqlDatabase::getInstance()->update("UPDATE mc_shops SET StartingCredit='{$_POST['starting_credit']}' WHERE Id='{$_SESSION['Index']->adminShop->getId()}' LIMIT 1");
			} else {
...
#region Hostname
			if ($_POST['custom_domain'] != '' && !isValidHostname($_POST['custom_domain'])) {
				...
				if ($_POST['custom_domain'] != '') {
					$UsedId = MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_shops WHERE Domain='{$_POST['custom_domain']}' LIMIT 1");
				}
				if (!$UsedId) {
					MySqlDatabase::getInstance()->update("UPDATE mc_shops SET Domain='{$_POST['custom_domain']}' WHERE Id='" . $_SESSION['Index']->adminShop->getShopInfo()->Id . "' LIMIT 1");
...
#region Image
		...
		if (isset($_POST['submit']) && !$errors) {
			...
			if (!$errors) {
				MySqlDatabase::getInstance()->query("UPDATE mc_shops SET ShopLogo = '$image_name' WHERE Id = '" . $_SESSION['Index']->adminShop->getShopInfo()->Id . "' LIMIT 1");
