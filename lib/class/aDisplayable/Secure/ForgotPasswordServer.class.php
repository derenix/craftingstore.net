...
class ForgotPasswordServer extends aDisplayable
{
	public function prepareDisplay()
	{
		function setResetToken($userId)
		{
			...
			MySqlDatabase::getInstance()->query("UPDATE mc_customers SET ResetPasswordToken='$token' WHERE Id='{$userId}' LIMIT 1");
			return $token;
		}

		function checkToken($userId, $token)
		{
			return ($token == MySqlDatabase::getInstance()->fetchOne("SELECT ResetPasswordToken FROM mc_customers WHERE Id='{$userId}'"));
		}

		function resetPassword($userId, $userEmail, $token)
		{
			$newPw = random_string_by_length(14);
			MySqlDatabase::getInstance()->query("UPDATE mc_customers SET Password='" . mysql_real_escape_string(bcrypt_encode($userEmail, $newPw)) . "', ResetPasswordToken = NULL WHERE Id='{$userId}' AND ResetPasswordToken='" . mysql_real_escape_string($token) . "' LIMIT 1");
		...
		{
			if (isNumber($userId)) {
				$user = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Email, ResetPasswordToken, FirstName, SurName FROM mc_customers WHERE Id='$userId' LIMIT 1");
				if ($user->Id == $userId)
					return $user;
			}
			return false;
		...
		if ($_GET['u'] && $_GET['t']) {
			...
			$user = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Email, FirstName, SurName FROM mc_customers WHERE Email='" . mysql_real_escape_string($_POST['email']) . "' LIMIT 1");
			if ($user->Id) {
