...
//Nimmt ein deserialisiertes Array aus der Datenbank an
		...
		if (!isNumber($ProdId, 1) || (($ProdId != '0') && (1 != MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_products WHERE Id='$ProdId' AND ShopId='$ShopId'")))) {
		...
		#region Bei einem vorhandenen Item gibts 'ne andere Überschrift und ggf. Startwerte
			...
			if (count($_POST) == 0) {
				$itemInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_products WHERE Id='$ProdId' AND ShopId='$ShopId' LIMIT 1");
				#region Allgemein
...
#region Speichern
		if ($save) {
			...
			if ($ProdId == 0) {
				...
				$newProductId = MySqlDatabase::getInstance()->insert("INSERT INTO mc_products (
			ShopId,
			Label,
			Description,
			Image,
		...
		VALUES (
			...
			#region Item wird geändert
				...
				$orgImageIsCustom = MySqlDatabase::getInstance()->fetchOne("SELECT CustomImage FROM mc_products WHERE ShopId='$ShopId' AND Id='$ProdId'");
				MySqlDatabase::getInstance()->query("UPDATE mc_products SET "
...
#region Cooldown
			...
			$OurEarningReate = 100 - MySqlDatabase::getInstance()->fetchOne("SELECT EarningRate FROM mc_customers WHERE Id='{$_SESSION['Index']->adminShop->getShopInfo()->CustomersId}' LIMIT 1");
