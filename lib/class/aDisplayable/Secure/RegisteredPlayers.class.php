...
class RegisteredPlayers extends aDisplayable
{
	...
	public function prepareDisplay()
	{
		...
		$this->pagination->prepare(MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_gamer AS g INNER JOIN mc_permittedshops AS p on p.GamerId=g.id AND p.ShopId='" . $_SESSION['Index']->adminShop->getId() . "'"), '?show=RegisteredPlayers' . $searchPagination, $first, $number);


		$players = array();
		foreach (MySqlDatabase::getInstance()->query("SELECT g.Id, g.Nickname, g.MinecraftName, SUM(ga.Difference+ga.BonusDifference) AS Revenue, p.BonusPoints AS Bonus
FROM mc_gamer AS g
