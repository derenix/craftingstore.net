...
class RegisterAdmin extends aDisplayable
{
	...
	private function resendMail()
	{
		...
		if (!$error && isset($_POST['mail'])) {
			$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, FirstName, SurName, Email, Password, Validated FROM mc_customers WHERE Email='" . mysql_real_escape_string($_POST['mail']) . "' LIMIT 1");
			if (!$UserInfo) {
				...
				#region Email als Bestätigung senden
					...
					MySqlDatabase::getInstance()->update("UPDATE mc_customers SET token='" . mysql_real_escape_string($token) . "' WHERE Id='{$UserInfo->Id}' LIMIT 1");
	...
	private function verifyCode()
	{
		if (!isNumber($_GET['u'])) {
			...
			$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id,Password,Email,FirstName,SurName,Token,Validated FROM mc_customers WHERE Id='{$_GET['u']}' LIMIT 1");
			if ($UserInfo->Validated) {
				$_SESSION['Index']->assign_say('VERIFICATION_ALREADY_DONE');
				$_SESSION['Index']->assign_say('VERIFICATION_CONFIRMED_TO_LOGIN');
				$_SESSION['Index']->assign_direct('SHOP_ID', $_GET['shop']);
			} elseif ($_GET['c'] == RegisterAdmin::generateValidationKey($UserInfo)) {
				MySqlDatabase::getInstance()->update("UPDATE mc_customers SET Validated='1',Token='' WHERE Id='{$UserInfo->Id}' LIMIT 1");
				MySqlDatabase::getInstance()->insert("INSERT INTO mc_customeraccounts (Time,Current,Difference,CustomersId) VALUES ('" . time() . "','0','0','{$UserInfo->Id}')");
	...
	private function registrationProcess()
	{
		if (count($_POST)) {
			...
			if (!$error) {
				...
				if ($newUserId) {
					$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, FirstName, SurName, Email, RegTime, Password FROM mc_customers WHERE Id='$newUserId' LIMIT 1");
	...
	private static function tryCreateUser($FirstName, $SurName, &$Email, $EmailAccept, $Password, $MinecraftName, &$Error)
	{
		...
		if ($Error == null) {
			...
			//Den User erstellen
			$insertId = MySqlDatabase::getInstance()->insert("INSERT INTO mc_customers (FirstName, SurName, Email, Password, RegTime, MinecraftName) VALUES ('" . mysql_real_escape_string($FirstName) . "','" . mysql_real_escape_string($SurName) . "','" . mysql_real_escape_string($Email) . "','" . mysql_real_escape_string($Password) . "','" . time() . "','" . mysql_real_escape_string($MinecraftName) . "')");
	...
	{
		return MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_customers WHERE email='" . mysql_real_escape_string($Email) . "' LIMIT 1");
	}
	#end
