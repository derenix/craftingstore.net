...
#region Wenn der User nicht eingeloggt/validiert ist, zum Login weiterleiten
		...
		foreach (MySqlDatabase::getInstance()->query("SELECT Money, Name, img FROM mc_moneypackages ORDER BY Money ASC LIMIT 5") as $row) {
		...
		$ShopId = (isNumber($_GET['shop']) ? MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_shops WHERE Id='{$_GET['shop']}'") : 0);
