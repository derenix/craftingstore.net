				...
				if (isset($_POST['amount'])) {
					...
					if (preg_match('/^([0-9]+[,\.]?[0-9]{0,2})$/', $amount)) {
						...
						if ($AccountCurrent >= $amount) {
							...
							MySqlDatabase::getInstance()->query("INSERT INTO mc_customeraccounts (CustomersId,Current,Difference,PayoutStatus,PayoutMail,Time) VALUES ('{$_SESSION['CustomerId']}','$newAmount','$amount','0','" . mysql_real_escape_string($PaypalMail) . "','" . time() . "')");
