...
#region Menüs
	...
	private function switchShop()
	{
		if (isNumber($_POST['NAVIGATION_CHANGE_SERVER'])) {
			foreach (MySqlDatabase::getInstance()->query("SELECT Id FROM mc_shops WHERE CustomersId='{$_SESSION['CustomerId']}'") as $row) {
				if ($_POST['NAVIGATION_CHANGE_SERVER'] == $row->Id) {
					...
					MySqlDatabase::getInstance()->query("UPDATE mc_customers SET LastShopId='{$row->Id}' WHERE Id='{$_SESSION['CustomerId']}' LIMIT 1");
					break;
				}
			}
		}
	...
	{
		$Shops = array();
		foreach (MySqlDatabase::getInstance()->query("SELECT Id, Label, Subdomain, Domain FROM mc_shops WHERE CustomersId='{$_SESSION['CustomerId']}'") as $row) {
			$Shops[] = array(
				'Id' => $row->Id,
				'Label' => $row->Label,
	...
	private function prepareAccount()
	{
		...
		$_SESSION['Index']->assign('ADM_ACCOUNT', str_replace('.', $_SESSION['Index']->say('COMMA'), sprintf('%01.2f', MySqlDatabase::getInstance()->fetchOne("SELECT Current FROM mc_customeraccounts WHERE CustomersId='{$_SESSION['CustomerId']}' ORDER BY Time DESC LIMIT 1") / 100)));
	}
