...
class Register extends aDisplayable
{
	...
	private function resendMail()
	{
		...
		if (!$error && isset($_POST['mail'])) {
			$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Nickname, Email, Password, Validated FROM mc_gamer WHERE Email='" . mysql_real_escape_string($_POST['mail']) . "' LIMIT 1");
			if (!$UserInfo) {
				...
				#region Email als Bestätigung senden
					...
					MySqlDatabase::getInstance()->update("UPDATE mc_gamer SET token='" . mysql_real_escape_string($token) . "' WHERE Id='{$UserInfo->Id}' LIMIT 1");
					$validationKey = Register::generateValidationKey($UserInfo, $token);
					if (isNumber($_GET['shop']) && (MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_shops WHERE Id='{$_GET['shop']}'"))) {
	...
	private function verifyCode()
	{
		if (!isNumber($_GET['u'])) {
			...
			$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id,Password,Email,Nickname,Token,Validated FROM mc_gamer WHERE Id='{$_GET['u']}' LIMIT 1");
			if ($UserInfo->Validated) {
				...
				MySqlDatabase::getInstance()->update("UPDATE mc_gamer SET Validated='1',Token='' WHERE Id='{$UserInfo->Id}' LIMIT 1");
				...
				MySqlDatabase::getInstance()->insert("INSERT INTO mc_gameraccounts (Time,GamerId,Action) VALUES ('" . time() . "','{$UserInfo->Id}','DEFAULT')");
	...
	{
		$time = time() - 172800;
		MySqlDatabase::getInstance()->query("DELETE FROM mc_permittedshops WHERE GamerId IN (
		SELECT Id FROM mc_gamer WHERE RegTime<'$time' AND Validated='0')");
		MySqlDatabase::getInstance()->query("DELETE FROM mc_gamer WHERE RegTime<'$time' AND Validated='0'");
	}
		...
		if (count($_POST)) {
			...
			if (!$error) { //nur versuchen, wenn bisher kein Fehler auftrat
				...
				if ($newUserId) {
					...
					$UserInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Nickname, Email, Password FROM mc_gamer WHERE Id='$newUserId' LIMIT 1");
					$token = random_string_by_length(15);
					MySqlDatabase::getInstance()->update("UPDATE mc_gamer SET token='" . mysql_real_escape_string($token) . "' WHERE Id='{$UserInfo->Id}' LIMIT 1");
					$validationKey = Register::generateValidationKey($UserInfo, $token);
					if (isNumber($_GET['shop']) && (MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_shops WHERE Id='{$_GET['shop']}'"))) {
