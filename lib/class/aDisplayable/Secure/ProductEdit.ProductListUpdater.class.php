...
class ProductListUpdater
{
	...
	public function load()
	{
		...
		foreach (MySqlDatabase::getInstance()->query("SELECT ProductId,Amount FROM mc_ProductsInProduct WHERE ParentProductId='{$this->ProductId}' AND ShopId='{$this->ShopId}'") as $row) {
	...
	public function save($newProductId = 0)
	{
		if ($newProductId > 0) {
			...
			MySqlDatabase::getInstance()->query("DELETE FROM mc_ProductsInProduct WHERE ParentProductId='$ProductId' AND ShopId='{$this->ShopId}'");
		}
		...
		if ($includedProducts) {
			MySqlDatabase::getInstance()->query("INSERT INTO mc_ProductsInProduct (ParentProductId,ProductId,ShopId,Amount) VALUES " . $includedProducts);
		}
	}

	private function getFullProductList()
	{
		foreach (MySqlDatabase::getInstance()->query("SELECT Id, Label FROM mc_products WHERE Id<>'{$this->ProductId}' AND ShopId='{$this->ShopId}' ORDER BY Label ASC") as $row) {
	...
	{
		foreach (MySqlDatabase::getInstance()->query("SELECT ProductId FROM mc_ProductsInProduct WHERE ParentProductId='$ProductId' AND (ShopId='$ShopId' OR ShopId='0')") as $row) {
