...
#region Profil Base-Url festlegen (entweder mit ShopId oder ohne)
		...
		$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT MinecraftName,Email,Nickname,NewEmail FROM mc_gamer WHERE Id='{$_SESSION['Index']->user->getLoginId()}' LIMIT 1");
		$default = true;
#region Änderung der E-Mail-Adresse abbrechen
		if (isset($_GET['editmail_cancel']))
			MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET NewEmail=null WHERE Id='{$_SESSION['Index']->user->getLoginId()}' LIMIT 1");
#end
#region E-Mail ändern
		if (isset($_GET['editmail'])) {
			$newMail = MySqlDatabase::getInstance()->fetchOne("SELECT NewEmail FROM mc_gamer WHERE Id='{$_SESSION['Index']->user->getLoginId()}' LIMIT 1");
			...
			if ($_GET['k'] && $newMail) {
				...
				elseif (MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer WHERE Id<>'{$_SESSION['Index']->user->getLoginId()}' AND (Email='$escapedmail' OR NewEmail='$escapedmail') LIMIT 1")) {
					$_SESSION['Index']->assign_say('PROFILE_EDITMAIL_ERROR', 'PROFILE_EDITMAIL_IN_USE');
				} //jetzt können wir Hand anlegen
				elseif ($mail) {
					MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET NewEmail='$escapedmail' WHERE Id='{$_SESSION['Index']->user->getLoginId()}'");
...
#region User aus einem Shop löschen
		elseif (isNumber($_GET['deleteshop'])) {
			$default = false;
			$ShopLabel = MySqlDatabase::getInstance()->fetchOne("SELECT s.Label FROM mc_permittedshops AS p INNER JOIN mc_shops AS s ON p.ShopId=s.Id WHERE p.GamerId='{$_SESSION['Index']->user->getLoginId()}' AND p.ShopId='{$_GET['deleteshop']}'");
			if ($ShopLabel) {
				$_SESSION['Index']->assign_direct('PROFILE_DO_DELETESHOP', 1);
				if (isset($_GET['commit'])) {
					if (MySqlDatabase::getInstance()->delete("DELETE FROM mc_permittedshops WHERE GamerId='{$_SESSION['Index']->user->getLoginId()}' AND ShopId='{$_GET['deleteshop']}' LIMIT 1"))
