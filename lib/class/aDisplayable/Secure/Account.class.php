...
class Account extends aDisplayable
{
	public function prepareDisplay()
	{
		...
		$AccountCurrent = MySqlDatabase::getInstance()->fetchOne("SELECT Current FROM mc_customeraccounts WHERE CustomersId='{$_SESSION['CustomerId']}' ORDER BY Time DESC LIMIT 1");
		$_SESSION['Index']->assign_direct('ADM_ACCOUNT_ACCOUNT_CURRENT', str_replace('.', $comma, sprintf('%01.2f', $AccountCurrent / 100)));
		$_SESSION['Index']->assign_direct('ADM_ACCOUNT_REVENUE_CURRENT_SHOP', str_replace('.', $comma, sprintf('%01.2f', MySqlDatabase::getInstance()->fetchOne("SELECT Sum(Difference) FROM mc_customeraccounts WHERE CustomersId='{$_SESSION['CustomerId']}' AND ShopId='{$_SESSION['Index']->adminShop->getId()}' AND PayoutStatus IS NULL") / 100)));
		$_SESSION['Index']->assign_direct('ADM_ACCOUNT_REVENUE_ALL_SHOP', str_replace('.', $comma, sprintf('%01.2f', MySqlDatabase::getInstance()->fetchOne("SELECT Sum(Difference) FROM mc_customeraccounts WHERE CustomersId='{$_SESSION['CustomerId']}' AND PayoutStatus IS NULL") / 100)));
		#endregion

		#region Kann eine Auszahlung beantragt werden?
		$PaypalMail = MySqlDatabase::getInstance()->fetchOne("SELECT PaypalMail FROM mc_customers WHERE Id='{$_SESSION['CustomerId']}' LIMIT 1");
		if (!$PaypalMail) {
			$_SESSION['Index']->assign_say('ADM_ACCOUNT_REQUEST_PAYOUT_NO_MAIL');
			$_SESSION['Index']->assign_say('ADM_ACCOUNT_TO_PROFILE');
		} else {
			$lastPayout = MySqlDatabase::getInstance()->fetchOne("SELECT Time FROM mc_customeraccounts WHERE CustomersId='{$_SESSION['CustomerId']}' AND PayoutStatus IS NOT NULL ORDER BY Time DESC LIMIT 1");
