...
class ItemListUpdater
{
	...
	public function load()
	{
		...
		if ($this->ProductId > 0) {
			foreach (MySqlDatabase::getInstance()->query("SELECT ItemId,Amount FROM mc_ItemsInProduct WHERE ProductId='{$this->ProductId}' AND ShopId='{$this->ShopId}'") as $row) {
	...
	public function save($newProductId = 0)
	{
		if ($newProductId > 0) {
			...
			MySqlDatabase::getInstance()->query("DELETE FROM mc_ItemsInProduct WHERE ProductId='$ProductId' AND ShopId='{$this->ShopId}'");
		}
		...
		if ($includedItems) {
			MySqlDatabase::getInstance()->query("INSERT INTO mc_ItemsInProduct (ShopId,ProductId,ItemId,Amount) VALUES " . $includedItems);
		}
	}
	...
	{
		foreach (MySqlDatabase::getInstance()->query("SELECT Id, Name FROM mc_items WHERE (ShopId='{$this->ShopId}' OR (ShopId='0' AND Id NOT IN (SELECT Id FROM mc_items WHERE ShopId='{$this->ShopId}'))) ORDER BY ShopId DESC, Name ASC") as $row) {
			// foreach(MySqlDatabase::getInstance()->query("SELECT Id, Name FROM mc_items WHERE ShopId='{$this->ShopId}' ORDER BY Name ASC") as $row){
	...
	{
		MySqlDatabase::getInstance()->query("INSERT IGNORE INTO mc_items (Id,ShopId,Name,Ingame,MineId,Damage,Lore,Image)
