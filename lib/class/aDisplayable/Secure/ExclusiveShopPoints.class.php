...
class ExclusiveShopPoints extends aDisplayable
{
	public function prepareDisplay()
	{
		...
		if ($_POST['submit']) {
			...
			$validPlayer = isNumber($playerId) && ($playerId == MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer AS g INNER JOIN mc_permittedshops AS p ON g.Id=p.GamerId WHERE g.Id='$playerId' AND p.ShopId='" . $_SESSION['Index']->adminShop->getId() . "' LIMIT 1"));
		...
		if ($anzeigen) {
			...
			$player = array();
			foreach (MySqlDatabase::getInstance()->query("SELECT g.Id,g.Nickname FROM mc_gamer AS g INNER JOIN mc_permittedshops AS p ON g.Id=p.GamerId WHERE p.ShopId='" . $_SESSION['Index']->adminShop->getId() . "'") as $row) {
			...
			$_SESSION['Index']->assign_say('ADM_BONUS_GIVEN', array(MySqlDatabase::getInstance()->fetchOne("SELECT Nickname FROM mc_gamer AS g INNER JOIN mc_permittedshops AS p ON g.Id=p.GamerId WHERE g.Id='$playerId' AND p.ShopId='" . $_SESSION['Index']->adminShop->getId() . "' LIMIT 1"), $points));
			$_SESSION['Index']->assign('ADM_BONUS_BACK_URL', '?show=RegisteredPlayers');
