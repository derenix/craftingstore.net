...
class ForgotPassword extends aDisplayable
{
	public function prepareDisplay()
	{
		function setResetToken($userId)
		{
			...
			MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET ResetPasswordToken='$token' WHERE Id='{$userId}' LIMIT 1");
			return $token;
		}

		function checkToken($userId, $token)
		{
			return ($token == MySqlDatabase::getInstance()->fetchOne("SELECT ResetPasswordToken FROM mc_gamer WHERE Id='{$userId}'"));
		}

		function resetPassword($userId, $userEmail, $token)
		{
			$newPw = random_string_by_length(14);
			MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET Password='" . mysql_real_escape_string(bcrypt_encode($userEmail, $newPw)) . "', ResetPasswordToken = NULL WHERE Id='{$userId}' AND ResetPasswordToken='" . mysql_real_escape_string($token) . "' LIMIT 1");
			return $newPw;
		}
		...
		{
			if (isNumber($userId)) {
				$user = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Nickname, Email, ResetPasswordToken FROM mc_gamer WHERE Id='$userId' LIMIT 1");
				if ($user->Id == $userId)
					return $user;
			}
			return false;
		...
		if ($_POST['text'] && !$error) {
			$user = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Nickname, Email FROM mc_gamer WHERE Nickname='" . mysql_real_escape_string($_POST['text']) . "' OR Email='" . mysql_real_escape_string($_POST['text']) . "' LIMIT 1");
			if ($user->Id) {
