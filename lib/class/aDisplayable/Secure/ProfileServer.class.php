	...
	public function prepareDisplay()
	{
		if (count($_POST)) {
			if ($_POST['passwordfirst'] || $_POST['passwordsecond']) {
				if ($_POST['passwordfirst'] == $_POST['passwordsecond']) {
					...
					if (User::validPassword($Pw)) {
						$Email = MySqlDatabase::getInstance()->fetchOne("SELECT Email FROM mc_customers WHERE Id='{$_SESSION['CustomerId']}'");
						$crypt = bcrypt_encode($Email, $Pw);
						MySqlDatabase::getInstance()->query("UPDATE mc_customers SET Password='" . mysql_real_escape_string($crypt) . "' WHERE Id='{$_SESSION['CustomerId']}'");
					} else {
			...
			if ($fehler) {
				...
				MySqlDatabase::getInstance()->query("UPDATE mc_customers SET PaypalMail='" . mysql_real_escape_string($_POST['paypal']) . "' WHERE Id='{$_SESSION['CustomerId']}'");
			}
		}
		...
		$customerInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Email,FirstName,SurName,PaypalMail,MinecraftName FROM mc_customers WHERE Id='{$_SESSION['CustomerId']}'");
