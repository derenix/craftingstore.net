		...
		*/
		...
		if ($_SESSION['CreateShop']['Step'] == 2) {
			...
			if ($_GET['newcode']) {
				...
				$player = MySqlDatabase::getInstance()->fetchOne("SELECT MinecraftName FROM mc_customers WHERE Id='" . mysql_real_escape_string($customerId) . "'");
				...
				$result = JSONquery::sendValidation($_SESSION['CreateShop']['hostname'],
					...
					$_SESSION['CreateShop']['apisalt'], $player, $msg, MySqlDatabase::getInstance()->getUUID());
		...
		if ($_SESSION['CreateShop']['Step'] == 3) {
			if (isset($_POST['serverlabel'])) {
				...
				if (!$_SESSION['CreateShop']['subdomain'] || !preg_match('/^[0-9a-z\-]+$/i', $_SESSION['CreateShop']['subdomain']) || MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_shops WHERE subdomain='" . mysql_real_escape_string($_SESSION['CreateShop']['subdomain']) . "'")) {
			...
			$templates = array();
			foreach (MySqlDatabase::getInstance()->query("SELECT Id, Label FROM mc_templates WHERE Public='1'") as $row) {
				$templates[] = array(
					'id' => $row->Id,
					'name' => $row->Label);
		...
		if ($_SESSION['CreateShop']['Step'] == 4) {
			...
			startTransaction();
			try {
				$RowId = MySqlDatabase::getInstance()->insert(sprintf("INSERT INTO mc_shops
			(Label, Subdomain, TemplateId, CustomersId, ServerHost, ServerPort, ServerUser, ServerPassword, ServerSalt) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s')",
				...
				MySqlDatabase::getInstance()->insert("INSERT INTO mc_productGroups (Id, ShopId, Label, lft, rgt, Enabled) VALUES ('1','$RowId','root','1','2','1')");
				commit();

				$_SESSION['Index']->adminShop = new Shop($RowId);
				#region Aktuelle Shopauswahl in der Datenbank speichern
				MySqlDatabase::getInstance()->query("UPDATE mc_customers SET LastShopId='$RowId' WHERE Id='{$_SESSION['CustomerId']}' LIMIT 1");
				#end
			} catch (Exception $e) {
				//@todo: log $e
