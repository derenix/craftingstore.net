	...
	public function prepareDisplay()
	{
		...
		if ($_GET['mode'] == 'edit' && $node) {
			...
			if (count($_POST)) {
				...
				} elseif (!isNumber($_POST['group'], true) || ($_POST['group'] != '0' && $_POST['group'] != MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_productGroups WHERE Id='{$_POST['group']}' LIMIT 1"))) {
			...
			#region Node bearbeiten
				...
				$nodeInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_productGroups WHERE Id='$node' AND ShopId='{$_SESSION['Index']->adminShop->getId()}'");
				if (!$nodeInfo) {
					...
					#endregion
					if ($save) {
						MySqlDatabase::getInstance()->query("UPDATE mc_productGroups SET Label='" . mysql_real_escape_string($_POST['label']) . "', Description='" . mysql_real_escape_string($_POST['description']) . "', Enabled='" . ($_POST['enabled'] ? 1 : 0) . "' WHERE ShopId='{$_SESSION['Index']->adminShop->getId()}' AND Id='$node' LIMIT 1");
		...
		#endregion
		#region Node löschen
		elseif ($_GET['mode'] == 'del' && $node && ($label = /*EIN GLEICHHEITSZEICHEN IST RICHTIG*/
				MySqlDatabase::getInstance()->fetchOne("SELECT Label FROM mc_productGroups WHERE Id='$node' AND ShopId='{$_SESSION['Index']->adminShop->getId()}'"))
		) {
			...
			if ($_GET['do']) {
				...
				MySqlDatabase::getInstance()->query("UPDATE mc_products SET Enabled='0' WHERE ShopId='{$_SESSION['Index']->adminShop->getId()}' AND GroupId NOT IN (SELECT Id FROM mc_productGroups WHERE ShopId='{$_SESSION['Index']->adminShop->getId()}')");
		...
		#region Übersicht anzeigen
			...
			if ($_GET['mode'] == 'copy') {
				MySqlDatabase::getInstance()->query("DELETE FROM mc_productGroups WHERE ShopId='{$_SESSION['Index']->adminShop->getId()}'");
				MySqlDatabase::getInstance()->query("INSERT INTO mc_productGroups (Id,ShopId,Label,lft,rgt,Description,Enabled) SELECT Id,'{$_SESSION['Index']->adminShop->getId()}',Label,lft,rgt,Description,Enabled FROM mc_productGroups WHERE ShopId='0'");
			}
			if ($_GET['mode'] == 'move' && $node && isNumber($lft = $_GET['lft']) && isNumber($rgt = $_GET['rgt'])) {
				if ($_GET['d'] == 'up' && ($node == MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_productGroups WHERE Id='$node' AND lft='$lft' AND rgt='$rgt'"))) {
					$this->nstree->moveLft($node);
				} elseif ($_GET['d'] == 'down' && ($node == MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_productGroups WHERE Id='$node' AND lft='$lft' AND rgt='$rgt'"))) {
