...
class TransferHistory extends aDisplayable
{
	...
	{
		//$productAmount = MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_transfers AS t WHERE t.ShopId='{$_SESSION['Index']->shop->getId()}' AND t.UserId='{$_SESSION['Index']->user->getLoginId()}'");
		$productAmount = MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_inventory AS i WHERE i.ShopId='{$_SESSION['Index']->shop->getId()}' AND i.GamerId='{$_SESSION['Index']->user->getLoginId()}'");
