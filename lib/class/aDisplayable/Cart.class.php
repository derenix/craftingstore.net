...
class Cart extends aDisplayable
{
	...
	public static function addToCart($ProdId, $Amount)
	{
		$_SESSION['LastAddedProduct'] = '';
		$_SESSION['LastAddedProductError'] = '';
		if (isNumber($ProdId) && isNumber($Amount) && ($prodInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Label, Image, Points, Enabled, DisableDuringCooldown FROM mc_products WHERE Id='$ProdId' AND ShopId='{$_SESSION['Index']->shop->getId()}'")) && $prodInfo->Enabled) {
