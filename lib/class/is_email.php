<?php /* https://code.google.com/p/isemail/downloads/list */
...
function is_email(&$email, $checkDNS = false, $errorlevel = false, &$parsedata = array())
{
	...
	$context_prior = ISEMAIL_COMPONENT_LOCALPART;
	$token = '';
	$token_prior = '';
	$parsedata = array(ISEMAIL_COMPONENT_LOCALPART => '', ISEMAIL_COMPONENT_DOMAIN => '');
	$atomlist = array(ISEMAIL_COMPONENT_LOCALPART => array(''), ISEMAIL_COMPONENT_DOMAIN => array(''));
	$element_count = 0;
	$element_len = 0;
	$hyphen_flag = false;
	$end_or_die = false;
	for ($i = 0; $i < $raw_length; $i++) {
		...
		switch ($context) {
			case ISEMAIL_COMPONENT_LOCALPART:
				switch ($token) {
					case ISEMAIL_STRING_OPENPARENTHESIS:
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_CFWS_COMMENT : ISEMAIL_DEPREC_COMMENT; else {
					...
					case ISEMAIL_STRING_DOT:
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_ERR_DOT_START : ISEMAIL_ERR_CONSECUTIVEDOTS; else if ($end_or_die) $return_status[] = ISEMAIL_DEPREC_LOCALPART;
						$end_or_die = false;
						$element_len = 0;
						$element_count++;
					...
					case ISEMAIL_STRING_HTAB:
						...
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_CFWS_FWS : ISEMAIL_DEPREC_FWS; else $end_or_die = true;
						$context_stack[] = $context;
					...
					case ISEMAIL_STRING_AT:
						...
						if ($parsedata[ISEMAIL_COMPONENT_LOCALPART] === '') $return_status[] = ISEMAIL_ERR_NOLOCALPART; elseif ($element_len === 0) $return_status[] = ISEMAIL_ERR_DOT_END;
						elseif (strlen($parsedata[ISEMAIL_COMPONENT_LOCALPART]) > 64) $return_status[] = ISEMAIL_RFC5322_LOCAL_TOOLONG;
					...
					default:
						if ($end_or_die) {
							...
							$ord = ord($token);
							if (($ord < 33) || ($ord > 126) || ($ord === 10) || (!is_bool(strpos(ISEMAIL_STRING_SPECIALS, $token)))) $return_status[] = ISEMAIL_ERR_EXPECTING_ATEXT;
							$parsedata[ISEMAIL_COMPONENT_LOCALPART] .= $token;
							$atomlist[ISEMAIL_COMPONENT_LOCALPART][$element_count] .= $token;
							$element_len++;
						}
			...
			case ISEMAIL_COMPONENT_DOMAIN:
				switch ($token) {
					case ISEMAIL_STRING_OPENPARENTHESIS:
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_DEPREC_CFWS_NEAR_AT : ISEMAIL_DEPREC_COMMENT; else {
							$return_status[] = ISEMAIL_CFWS_COMMENT;
							$end_or_die = true;
						}
						$context_stack[] = $context;
						...
						break;
					case ISEMAIL_STRING_DOT:
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_ERR_DOT_START : ISEMAIL_ERR_CONSECUTIVEDOTS; elseif ($hyphen_flag) $return_status[] = ISEMAIL_ERR_DOMAINHYPHENEND;
						else if ($element_len > 63) $return_status[] = ISEMAIL_RFC5322_LABEL_TOOLONG;
						$end_or_die = false;
						$element_len = 0;
						$element_count++;
					...
					case ISEMAIL_STRING_HTAB:
						...
						if ($element_len === 0) $return_status[] = ($element_count === 0) ? ISEMAIL_DEPREC_CFWS_NEAR_AT : ISEMAIL_DEPREC_FWS; else {
					...
					default:
						...
						if (($ord < 33) || ($ord > 126) || (!is_bool(strpos(ISEMAIL_STRING_SPECIALS, $token)))) {
							...
							if ($element_len === 0) {
								$return_status[] = ISEMAIL_ERR_DOMAINHYPHENSTART;
							}
							$hyphen_flag = true;
				...
				switch ($token) {
					case ISEMAIL_STRING_CLOSESQBRACKET:
						if ((int)max($return_status) < ISEMAIL_DEPREC) {
							...
							if (preg_match('/\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/', $addressliteral, $matchesIP) > 0) {
								...
								if ($index !== 0) $addressliteral = substr($addressliteral, 0, $index) . '0:0';
							}
							if ($index === 0) {
								...
								if ($index === false) {
									if ($groupCount !== $max_groups) $return_status[] = ISEMAIL_RFC5322_IPV6_GRPCOUNT;
								} else {
									if ($index !== strrpos($IPv6, ISEMAIL_STRING_DOUBLECOLON)) $return_status[] = ISEMAIL_RFC5322_IPV6_2X2XCOLON; else {
										if ($index === 0 || $index === (strlen($IPv6) - 2)) $max_groups++;
										if ($groupCount > $max_groups) $return_status[] = ISEMAIL_RFC5322_IPV6_MAXGRPS; elseif ($groupCount === $max_groups) $return_status[] = ISEMAIL_RFC5321_IPV6DEPRECATED;
									}
								}
								if ((substr($IPv6, 0, 1) === ISEMAIL_STRING_COLON) && (substr($IPv6, 1, 1) !== ISEMAIL_STRING_COLON)) $return_status[] = ISEMAIL_RFC5322_IPV6_COLONSTRT; elseif ((substr($IPv6, -1) === ISEMAIL_STRING_COLON) && (substr($IPv6, -2, 1) !== ISEMAIL_STRING_COLON)) $return_status[] = ISEMAIL_RFC5322_IPV6_COLONEND;
								elseif (count(preg_grep('/^[0-9A-Fa-f]{0,4}$/', $matchesIP, PREG_GREP_INVERT)) !== 0) $return_status[] = ISEMAIL_RFC5322_IPV6_BADCHAR;
								else $return_status[] = ISEMAIL_RFC5321_ADDRESSLITERAL;
							}
						} else $return_status[] = ISEMAIL_RFC5322_DOMAINLITERAL;
						$parsedata[ISEMAIL_COMPONENT_DOMAIN] .= $token;
						$atomlist[ISEMAIL_COMPONENT_DOMAIN][$element_count] .= $token;
						$element_len++;
						$context_prior = $context;
					...
					default:
						...
						if (($ord > 127) || ($ord === 0) || ($token === ISEMAIL_STRING_OPENSQBRACKET)) {
							$return_status[] = ISEMAIL_ERR_EXPECTING_DTEXT;
							break;
			...
			case ISEMAIL_CONTEXT_QUOTEDSTRING:
				switch ($token) {
					...
					default:
						...
						} elseif (($ord < 32) || ($ord === 127)) $return_status[] = ISEMAIL_DEPREC_QTEXT;
						$parsedata[ISEMAIL_COMPONENT_LOCALPART] .= $token;
						$atomlist[ISEMAIL_COMPONENT_LOCALPART][$element_count] .= $token;
						$element_len++;
				}
			...
			case ISEMAIL_CONTEXT_QUOTEDPAIR:
				$ord = ord($token);
				if ($ord > 127) $return_status[] = ISEMAIL_ERR_EXPECTING_QPAIR; elseif ((($ord < 31) && ($ord !== 9)) || ($ord === 127)) $return_status[] = ISEMAIL_DEPREC_QP;
				$context_prior = $context;
				$context = (int)array_pop($context_stack);
			...
			case ISEMAIL_CONTEXT_COMMENT:
				switch ($token) {
					...
					default:
						$ord = ord($token);
						if (($ord > 127) || ($ord === 0) || ($ord === 10)) {
							$return_status[] = ISEMAIL_ERR_EXPECTING_CTEXT;
							break;
			...
			case ISEMAIL_CONTEXT_FWS:
				if ($token_prior === ISEMAIL_STRING_CR) {
					if ($token === ISEMAIL_STRING_CR) {
						$return_status[] = ISEMAIL_ERR_FWS_CRLF_X2;
						break;
					}
					if (isset($crlf_count)) {
						if (++$crlf_count > 1) $return_status[] = ISEMAIL_DEPREC_FWS;
					} else $crlf_count = 1;
				}
				switch ($token) {
					case ISEMAIL_STRING_CR:
						if ((++$i === $raw_length) || ($email[$i] !== ISEMAIL_STRING_LF)) $return_status[] = ISEMAIL_ERR_CR_NO_LF;
						break;
					case ISEMAIL_STRING_SP:
					case ISEMAIL_STRING_HTAB:
						break;
		...
		if ((int)max($return_status) > ISEMAIL_RFC5322) break;
	}
	if ((int)max($return_status) < ISEMAIL_RFC5322) {
		if ($context === ISEMAIL_CONTEXT_QUOTEDSTRING) $return_status[] = ISEMAIL_ERR_UNCLOSEDQUOTEDSTR; elseif ($context === ISEMAIL_CONTEXT_QUOTEDPAIR) $return_status[] = ISEMAIL_ERR_BACKSLASHEND;
		elseif ($context === ISEMAIL_CONTEXT_COMMENT) $return_status[] = ISEMAIL_ERR_UNCLOSEDCOMMENT;
		elseif ($context === ISEMAIL_COMPONENT_LITERAL) $return_status[] = ISEMAIL_ERR_UNCLOSEDDOMLIT;
		elseif ($token === ISEMAIL_STRING_CR) $return_status[] = ISEMAIL_ERR_FWS_CRLF_END;
		elseif ($parsedata[ISEMAIL_COMPONENT_DOMAIN] === '') $return_status[] = ISEMAIL_ERR_NODOMAIN;
		elseif ($element_len === 0) $return_status[] = ISEMAIL_ERR_DOT_END;
		elseif ($hyphen_flag) $return_status[] = ISEMAIL_ERR_DOMAINHYPHENEND;
		elseif (strlen($parsedata[ISEMAIL_COMPONENT_DOMAIN]) > 255) $return_status[] = ISEMAIL_RFC5322_DOMAIN_TOOLONG;
		elseif (strlen($parsedata[ISEMAIL_COMPONENT_LOCALPART] . ISEMAIL_STRING_AT . $parsedata[ISEMAIL_COMPONENT_DOMAIN]) > 254) $return_status[] = ISEMAIL_RFC5322_TOOLONG;
	...
	if ($checkDNS && ((int)max($return_status) < ISEMAIL_DNSWARN) && function_exists('dns_get_record')) {
		if ($element_count === 0) $parsedata[ISEMAIL_COMPONENT_DOMAIN] .= '.';
		$result = @dns_get_record($parsedata[ISEMAIL_COMPONENT_DOMAIN], DNS_MX);
		if ((is_bool($result) && !(bool)$result)) $return_status[] = ISEMAIL_DNSWARN_NO_RECORD; else {
			if (count($result) === 0) {
				$return_status[] = ISEMAIL_DNSWARN_NO_MX_RECORD;
				$result = @dns_get_record($parsedata[ISEMAIL_COMPONENT_DOMAIN], DNS_A + DNS_CNAME);
				if (count($result) === 0) $return_status[] = ISEMAIL_DNSWARN_NO_RECORD;
	...
	if (!$dns_checked && ((int)max($return_status) < ISEMAIL_DNSWARN)) {
		...
		if (is_numeric($atomlist[ISEMAIL_COMPONENT_DOMAIN][$element_count][0])) $return_status[] = ISEMAIL_RFC5321_TLDNUMERIC;
	}
	$return_status = array_unique($return_status);
	$final_status = (int)max($return_status);
	if (count($return_status) !== 1) array_shift($return_status);
