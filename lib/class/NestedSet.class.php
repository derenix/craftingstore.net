...
#region Methods
	...
	{
		if (!$this->locked) {
			MySqlDatabase::getInstance()->query("LOCK TABLES {$this->table} WRITE, {$this->table} AS n WRITE, {$this->table} AS p WRITE");
			$this->locked = true;
		}
	}
	 ...
	 * $rgt=MySqlDatabase::getInstance()->fetchOne($sql);
	 * $lft=$rgt+1;
	 * $rgt=$lft+1;
	 *
	...
	{
		$this->_LockTables();
		MySqlDatabase::getInstance()->query("UPDATE {$this->table} SET rgt=rgt+2 WHERE ShopId='{$this->shop->getId()}' AND rgt>='$rgt'");
		MySqlDatabase::getInstance()->query("UPDATE {$this->table} SET lft=lft+2 WHERE ShopId='{$this->shop->getId()}' AND lft>'$rgt'");
		$id = MySqlDatabase::getInstance()->insert("INSERT INTO {$this->table} ({$this->name},lft,rgt,ShopId,Enabled,Description) VALUES ('" . mysql_real_escape_string($name) . "','$rgt','$rgt'+1,'{$this->shop->getId()}','" . ($enabled ? 1 : 0) . "','" . mysql_real_escape_string($description) . "')");
		$this->_UnlockTables();
		return $id;
	}
	...
	{
		return MySqlDatabase::getInstance()->fetchOneRow("SELECT n.*, COUNT(*)-1 AS Level FROM {$this->table} AS n, {$this->table} AS p
	...
	{
		return MySqlDatabase::getInstance()->fetchOneRow("SELECT n.{$this->pk},n.{$this->name},n.lft,n.rgt FROM {$this->table} AS n LEFT JOIN {$this->table} AS p ON p.{$this->pk}='$id' WHERE n.lft<p.lft AND n.rgt>p.rgt AND n.ShopId='{$this->shop->getId()}' AND p.ShopId='{$this->shop->getId()}' ORDER BY (n.rgt-p.rgt) ASC LIMIT 1");
	}
...
WHERE n.lft BETWEEN {$p->lft}+1 AND {$p->rgt}-1 AND {$this->pk}='$childNode' AND ShopId='{$this->shop->getId()}'";
		return (bool)MySqlDatabase::getInstance()->fetchOne($sql);
	}
	...
	public function containsImmediateName($parentNode, $childNodeName, $excludeNode)
	{
		if (!isNumber($parentNode) || !isNumber($excludeNode)) {
			...
			$p = $this->getNode($parentNode);
			return MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM (
	 ...
	 * @param bool $enabled
	 * @param null $description
	 * @internal param int $id id of the node
	...
	public function deleteNode($id)
	{
		...
		foreach ($sql as $query)
			MySqlDatabase::getInstance()->query($query);
		$this->_UnlockTables();
		return true;
	}
	 ...
	 * @param $newParent
	 * @internal param int $siblingTarget id of the node to move the node after
	 * @return true
	 */
	...
	protected function getIdRgt($rgt)
	{
		...
		$pk = MySqlDatabase::getInstance()->fetchOne($sql);
		if (!$pk) {
			return false;
		}
		return $pk;
