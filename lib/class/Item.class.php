...
class Item
{
	...
	public static function getItemInfoById($ItemId, &$ConnectedProducts)
	{
		...
		if (!$itemInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_products WHERE Id='$ItemId' AND ShopId='$ShopId'")) {
			return null;
		}
		#end
		#region Informationen zu verknüpften Produkten ermitteln
		$ConnectedProducts = array();
		foreach (MySqlDatabase::getInstance()->query("SELECT p.Label, pip.Amount, p.Image FROM mc_ProductsInProduct AS pip LEFT JOIN mc_products AS p ON p.Id=pip.ProductId AND p.ShopId='$ShopId' WHERE pip.ParentProductId='$ItemId' AND pip.ShopId='$ShopId'") as $row) {
		...
		foreach (MySqlDatabase::getInstance()->query("SELECT i.Name, iip.Amount, i.Image FROM mc_ItemsInProduct AS iip LEFT JOIN mc_items AS i ON i.Id=iip.ItemId AND i.ShopId='$ShopId' WHERE iip.ProductId='$ItemId' AND iip.ShopId='$ShopId'") as $row) {
	...
	public function updateItemInfo()
	{
		...
		if (!isset($this->itemID)) {
			...
			$this->itemInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_products WHERE Id='{$this->itemID}'");
		}
	}
	...
	{
		$ExpiredItem = MySqlDatabase::getInstance()->fetchOneRow("
	...
	{
		if ($disabled = MySqlDatabase::getInstance()->fetchOne("SELECT DisabledUntil FROM mc_inventory WHERE ShopId='$ShopId' AND GamerId='$GamerId' AND ProductId='$ProdId' ORDER BY DisabledUntil DESC LIMIT 1")) {
	...
	public static function CheckLabelAllowed($ShopId, $label, $id = '-1')
	{
		...
		foreach (MySqlDatabase::getInstance()->query(Item::GetItemQuery($ShopId)) as $row) {
	...
	public static function CheckMenueAllowed($ShopId, $menu)
	{
		...
		if (!isNumber($ShopId) || !isNumber($menu, true) || ($menu != MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_productGroups WHERE Id='$menu' AND ShopId='$ShopId'")))
			return false;
		return true;
	}
	#end
	...
	public static function CheckItemSetItemsAllowed($ShopId, $itemAmount, $itemId, &$checkedAmount)
	{
		...
		$max = MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_products WHERE ShopId='0' OR ShopId='$ShopId'");
		#end
		...
		for ($i = 0; $i < $count_itemId; $i++) {
			...
			if (isNumber($cur_id)) {
				...
				if (MySqlDatabase::getInstance()->fetchOne("SELECT COUNT(*) FROM mc_products WHERE Id='$cur_id' AND (ShopId='$ShopId' OR ShopId='0')") > 0) #Suche nach der Id in den ShopItems bzw. bei den Default-Items
				{
	...
	public static function GetItemName($ShopId, $ItemId)
	{
		if (!isNumber($ItemId) || !isNumber($ShopId))
			return null;
		return MySqlDatabase::getInstance()->fetchOne("SELECT Label FROM mc_products WHERE Id='$ItemId' AND ShopId='$ShopId'");
	}
	#end
