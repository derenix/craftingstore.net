...
class User
{
	...
	{
		if (isNumber($userId)) {
			$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Id, Nickname, Validated FROM mc_gamer WHERE Id='$userId' LIMIT 1");
			$this->Id = $userInfo->Id;
	...
	{
		if (isNumber($Id)) {
			$userdata = MySqlDatabase::getInstance()->fetchOneRow("SELECT * FROM mc_gamer WHERE Id = '$Id'");
			return $userdata;
		}
		return false;
	}
	...
	public function tryLogin($name, $pw, $shopId, &$WaitTime)
	{
		...
		$row = MySqlDatabase::getInstance()->fetchOneRow("SELECT Password, Id, Email, Nickname, Validated FROM mc_gamer WHERE Nickname='" . mysql_real_escape_string($name) . "' OR Email='" . mysql_real_escape_string($name) . "' LIMIT 1");
		if (!$row)
			return -2;
		...
		if (bcrypt_check($row->Email, $pw, $row->Password)) {
			MySqlDatabase::getInstance()->query("DELETE FROM mc_loginerrors WHERE IP='{$_SERVER['REMOTE_ADDR']}'");
			MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET IsLoggedIn='1', ResetPasswordToken=NULL, LastLang='" . $_SESSION['Index']->lang->getLangId() . "' WHERE Id='{$row->Id}'");
			$this->Id = $row->Id;
			...
			if (!$shopId || MySqlDatabase::getInstance()->fetchOne("SELECT RegTime FROM mc_permittedshops WHERE GamerId='{$row->Id}' AND ShopId='$shopId' LIMIT 1")) {
				return 0;
	...
	{
		return User::isPassword(MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer WHERE Nickname='" . mysql_real_escape_string($name) . "' OR Email='" . mysql_real_escape_string($name) . "' LIMIT 1"), $pw);
	}
	#end
	...
	{
		if (isNumber($id)) {
			$row = MySqlDatabase::getInstance()->fetchOneRow("SELECT Email, Password FROM mc_gamer WHERE Id='$id' LIMIT 1");
		...
		if (User::validPassword($newPw)) {
			if (isNumber($id)) {
				...
				if ($row->Email) {
					...
					{
						MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET Password='" . mysql_real_escape_string(bcrypt_encode($row->Email, $newPw)) . "' WHERE Id='$id'");
						return 0;
					}
	...
	public function createToken()
	{
		...
		MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET Token='$token' WHERE Id='{$this->Id}'");
		return $token;
	}
	#end
	...
	public function Logout($id = null)
	{
		...
		MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET IsLoggedIn='0' WHERE Id='$id'");
	...
	{
		return (($this->Id > 0) && MySqlDatabase::getInstance()->fetchOne("SELECT IsLoggedIn FROM mc_gamer WHERE Id='{$this->Id}'"));
	}
	#end
	...
	# Bei Erfolg wird der Wert von MySqlDatabase::getInstance()->insert(); zurückgegeben
	public static function tryCreateUser($Nickname, &$Email, $EmailAccept, &$Password, $MinecraftName, &$Error)
	{
		$Error = null;
		// MySqlDatabase::getInstance()->delete("DELETE FROM mc_gamer WHERE Validated='0' AND RegTime<'".(time() - 86400)."'");
		...
		if ($Error == null) {
			...
			//Den User erstellen
			$insertId = MySqlDatabase::getInstance()->insert("INSERT INTO mc_gamer (Nickname, Email, Password, MinecraftName, RegTime) VALUES ('" . mysql_real_escape_string($Nickname) . "','" . mysql_real_escape_string($Email) . "','" . mysql_real_escape_string($Password) . "','" . mysql_real_escape_string($MinecraftName) . "','" . time() . "')");
			if (!$insertId) {
				$Error['db'] = 1;
				return false;
			}
	...
	public static function addPermittedShop($ShopId, $UserId)
	{
		if (!isNumber($ShopId) || !isNumber($UserId))
			return false;
		try {
			MySqlDatabase::getInstance()->insert("INSERT INTO mc_permittedshops (GamerId, ShopId, BonusPoints) SELECT '$UserId','$ShopId',StartingCredit FROM mc_shops WHERE Id='$ShopId'");
			return true;
		} catch (Exception $e) {
			echo $e;
			return false;
	...
	public static function sendValidationMail($ShopId, $UserId, $validationType = null)
	{
		...
		if ($validationType == 2) { #Passwort vergessen
			$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Nickname, Email, Password
FROM mc_gamer WHERE Id='$UserId'");
	...
	public function sendChangeMailValidation($NewEmail, $ShopId)
	{
		...
		$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Nickname, NewEmail, Password
FROM mc_gamer
WHERE Id='{$this->Id}'");
	...
	{
		$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Email, Password, NewEmail FROM mc_gamer WHERE Id='{$this->Id}'");
		...
		if ($key != User::getChangeMailValidationKey($userInfo->NewEmail, $userInfo->Password))
			return -3;
		//Alles gut
		if (MySqlDatabase::getInstance()->update("UPDATE mc_gamer SET Email=NewEmail, NewEmail=null, Password='" . mysql_real_escape_string(bcrypt_encode($userInfo->NewEmail, $pw)) . "' WHERE Id='{$this->Id}'"))
			return true;
	}
	...
	public static function validateUser($ShopId, $UserId, $Code)
	{
		...
		$userInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT g.Nickname, g.Email, g.Password, g.Validated, p.RegTime FROM mc_permittedshops AS p LEFT JOIN mc_gamer AS g ON g.Id=p.GamerId WHERE p.GamerId='$UserId' AND p.ShopId='$ShopId'");
		...
		if ((strtotime($userInfo->RegTime) + User::$verificationTime) < time()) {
			MySqlDatabase::getInstance()->query("DELETE FROM mc_gamer WHERE Id='$UserId' LIMIT 1");
			MySqlDatabase::getInstance()->query("DELETE FROM mc_gameraccounts WHERE GamerId='$UserId' LIMIT 1");
			MySqlDatabase::getInstance()->query("DELETE FROM mc_permittedshops WHERE GamerId='$UserId' LIMIT 1");
			return 5;
		}
		...
		//Code ist gültig
		MySqlDatabase::getInstance()->query("UPDATE mc_gamer SET Validated='1' WHERE GamerId='$UserId' LIMIT 1");
		return 0;
	}
	#end
	...
	{
		$row = MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer WHERE Nickname='" . mysql_real_escape_string($Nickname) . "' LIMIT 1");
		if ($row)
			return $row;
		return 0;
	}
	...
	{
		$row = MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer WHERE Email='" . mysql_real_escape_string($Email) . "' LIMIT 1");
		if ($row)
			return $row;
		return 0;
	}
	...
	{
		$row = MySqlDatabase::getInstance()->fetchOne("SELECT Id FROM mc_gamer WHERE Minecraftname='" . mysql_real_escape_string($Minecraftname) . "' LIMIT 1");
		if ($row)
			return $row;
		return 0;
	}
	...
	{
		return MySqlDatabase::getInstance()->fetchOne("SELECT PlayerOnline FROM mc_permittedshops WHERE GamerId='{$this->Id}' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1");
	}

	public function setPlayerOnlineStatus($isOnline)
	{
		MySqlDatabase::getInstance()->update("UPDATE mc_permittedshops SET PlayerOnline='" . ($isOnline ? '1' : '0') . "' WHERE GamerId='{$this->Id}' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1");
	}
		...
		if (is_array($ProductId)) {
			...
			for ($i = 0; $i < count($ProductId); $i++) {
				if ($TransferId = $this->addProductToInventory($ProductId[$i], $Amount[$i], $error, false)) {
					...
					MySqlDatabase::getInstance()->query("UPDATE mc_products SET BuyCounter=BuyCounter+{$Amount[$i]}, Revenue=Revenue+$revenue WHERE Id='{$ProductId[$i]}' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1");
			...
			$pointsPerItem = MySqlDatabase::getInstance()->fetchOne("SELECT Points FROM mc_products WHERE Id='$ProductId' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1");
			...
			if ($TransferId = $this->addProductToInventory($ProductId, $Amount, $error, false)) {
				...
				MySqlDatabase::getInstance()->query("UPDATE mc_products SET BuyCounter=BuyCounter+$Amount, Revenue=Revenue+$revenue WHERE Id='$ProductId' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1");
				commit();
				return $TransferId;
			} else {
				rollback();
	...
	{
		if (!$ProductInfo = MySqlDatabase::getInstance()->fetchOneRow("SELECT Cooldown, CooldownInterval, DisableDuringCooldown FROM mc_products WHERE Id='$ProductId' AND ShopId='{$_SESSION['Index']->shop->getId()}' LIMIT 1")) {
			if ($lock) rollback();
		...
		return MySqlDatabase::getInstance()->insert("INSERT INTO mc_inventory (ShopId,GamerId,ProductId,Amount,Cooldown,CooldownInterval) VALUES ('{$_SESSION['Index']->shop->getId()}','{$this->Id}','$ProductId','$Amount'," . ($ProductInfo->DisableDuringCooldown ? "'{$ProductInfo->Cooldown}','{$ProductInfo->CooldownInterval}')" : "null,null)"));
		#end
	}
	#end
...
#region Inpayment($GamerId, $addEuro)
	...
	public static function Inpayment($GamerId, $addEuro)
	{
		...
		MySqlDatabase::getInstance()->query("INSERT INTO mc_gameraccounts (Time,GamerId,Current,Difference,Revenue,Action) SELECT '" . time() . "','$GamerId', Current+$addPoints, $addPoints, $addCents, 'INPAYMENT' FROM mc_gameraccounts WHERE GamerId='$GamerId' ORDER BY Time DESC LIMIT 1");
	}
	#end
	...
	public static function givePlayerBonusPoints($UserId, $ShopId, $addPoints)
	{
		if (isNumber($addPoints)) {
			...
			MySqlDatabase::getInstance()->query("UPDATE mc_permittedshops SET BonusPoints=BonusPoints+$addPoints WHERE GamerId='$UserId' AND ShopId='$ShopId'");
			MySqlDatabase::getInstance()->insert("INSERT INTO mc_gameraccounts (Time,GamerId,Current,Difference,BonusDifference,Action,ShopId)
	...
	public static function debitPlayer($UserId, $ShopId, $debitPoints, $InventoryId = 0, $lock = true)
	{
		...
		if ($bonusPoints > 0) {
			MySqlDatabase::getInstance()->query("UPDATE mc_permittedshops SET BonusPoints=$newBonusPoints WHERE GamerId='$UserId' AND ShopId='$ShopId' LIMIT 1");
		}
		...
		$insertId = MySqlDatabase::getInstance()->insert("INSERT INTO mc_gameraccounts (GamerId,Current,Difference,Revenue,BonusDifference,Action,ShopId,InventoryId,Time) VALUES ('$UserId','$newPoints','$subtractNormalPoints','$normalPointsAsCents','$subtractBonusPoints','BOUGHT_ITEM','$ShopId','$InventoryId','" . time() . "')");
		#end
		#region Eintragen erfolgreich => Verdienst buchen
		if ($insertId > 0) {
			$CustomersId = $_SESSION['Index']->shop->getShopInfo()->CustomersId;
			$EarningRate = MySqlDatabase::getInstance()->fetchOne("SELECT EarningRate FROM mc_customers WHERE Id='$CustomersId' LIMIT 1");
			...
			MySqlDatabase::getInstance()->insert("INSERT INTO mc_customeraccounts (CustomersId,Current,Difference,ShopId,Time) SELECT '$CustomersId',c.Current+$customersGewinn,'$customersGewinn','{$_SESSION['Index']->shop->getId()}','" . time() . "' FROM mc_customeraccounts AS c WHERE c.CustomersId='$CustomersId' ORDER BY c.Time DESC LIMIT 1");
			MySqlDatabase::getInstance()->insert("INSERT INTO mc_ouraccount (Current,Difference,Time) SELECT o.Current+$unserGewinn,'$unserGewinn','" . time() . "' FROM mc_ouraccount AS o ORDER BY o.Time DESC LIMIT 1");
	...
	{
		return MySqlDatabase::getInstance()->fetchOne("SELECT BonusPoints FROM mc_permittedshops WHERE GamerId='$GamerId' AND ShopId='$ShopId' LIMIT 1");
	}
	#end
	#region bonusPoints()
	// Ermittelt die Bonuspunkte des aktuellen Spielers
	...
	{
		return MySqlDatabase::getInstance()->fetchOne("SELECT Current FROM mc_gameraccounts WHERE GamerId='$GamerId' ORDER BY Time DESC LIMIT 1");
	}
	#end
	#region normalPoints()
	// Ermittelt die Bonuspunkte des aktuellen Spielers
